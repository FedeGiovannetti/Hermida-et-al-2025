...
# 1. Table 1. Descriptive statistics for each study group and assessment time
...
```{r}
  ...
  set_caption(caption= as_paragraph(as_chunk("Table 1",
...
Descriptive %>%                                                                 #Saving table to "Tables" folder
  save_as_docx(path = "Tables/Table_01-Descriptive.docx", pr_section = sect_properties)
...
### 3.1. Table 2. Generalized Mixed Model results and model fitness values
...
model_table %>%
  save_as_docx(path = "Tables/Table02.docx", pr_section = sect_properties)
...
for (i in 1:length(significant_questions)) {
      ...
      plot_list[[i]] <- Frecuencias_conceptos%>%  
        ...
        geom_errorbar(aes(ymin=Porcentaje-SE, ymax=Porcentaje+SE),
                      position=position_dodge(width = 0.2),width=0,lwd=1.75)+
        annotate("text", label = paste(strwrap(significant_questions[i], width = 45), collapse = "\n"),
